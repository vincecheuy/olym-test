...
# data  = pd.DataFrame(pd.read_csv('price_data.csv'))
# for infile in glob.glob("/Tom-Demark-Indicator-master/Data/*.csv"):
#     data = pd.DataFrame(pd.read_csv(infile))
# # data  = pd.DataFrame(pd.read_csv("/Tom-Demark-Indicator-master/Data/ABT.CSV"))
# #rename columns
#     data.columns = ['Date','Open','High','Low',
...
def main():
    p = pathlib2.Path(".")
    print("syspath==" + os.path.abspath(__file__))
    for infile in glob.glob("D:\PythonProjects\Tom-Demark-Indicator-master1/Data/*.csv"):
    # for infile in p.glob("Data/*.csv"):
        data = pd.DataFrame(pd.read_csv(infile))
    ...
    #If count > 0, a price flip is not required for the count to progress.
        ...
        # plt.ylim(0, h[idx_max] * 1.14)
        #
        # #show a grid on plot
        # ax1.grid(True,color='#2E2E2E',linestyle='--')
        #
        # #generate candlesticks
        # lines, patches = candlestick_ohlc(ax1,ohlc,width=0.6,colorup='g',colordown='r')
        ...
        #     size=8, weight='bold')
        signal =[]
        # signal = np.vstack((longVal,shortVal,buyVal,agbuyVal,sellVal,agsellVal))
        # signals = pd.DataFrame(signal.T, columns=["longVal","shortVal","buyVal","agbuyVal","sellVal","agsellVal"],index=t)
        signal = np.vstack((longVal,shortVal))
        signals = pd.DataFrame(signal.T, columns=["longVal","shortVal"],index=t)
        if 9 in signals.iloc[-1].values:
            new_file = infile.replace(".csv",".txt")
            print(new_file)
            signals.iloc[-1].to_csv(new_file)


# if longVal.iloc[-1] == 9:
    #
    # print(longVal, shortVal,buyVal,agbuyVal, sellVal, agsellVal)
    ...
    # plt.subplots_adjust(left=0.06,right=0.94,bottom=0,
    #                     top=0.94,wspace=0.20,hspace=0.80)
    #
